...
- (nonnull AVPlayerItem *)playerItemWithCreationOptions:(nonnull FVPCreationOptions *)options {
  ...
  // LOCAL PATCH: optional forward-buffer cap (see VideoPlayerOptions
  // .forwardBufferDuration). nil leaves AVPlayer's automatic (aggressive)
  // buffering; a value (seconds) limits read-ahead to save bandwidth.
  if (options.forwardBufferDurationMs != nil) {
    item.preferredForwardBufferDuration = options.forwardBufferDurationMs.doubleValue / 1000.0;
  }
  return item;
}
