...
public abstract class VideoPlayer implements VideoPlayerInstanceApi {
  ...
  /**
   * LOCAL PATCH: builds a {@link LoadControl} that caps forward buffering at {@code
   * forwardBufferDurationMs}, or returns null to keep ExoPlayer's default (~50s).
   *
   * <p>Exposes {@code VideoPlayerOptions.forwardBufferDuration} so abandoned/seek-away playback
   * doesn't keep downloading. Playback-start thresholds are clamped so they never exceed the cap
   * (a requirement of {@link DefaultLoadControl}).
   */
  @UnstableApi
  @Nullable
